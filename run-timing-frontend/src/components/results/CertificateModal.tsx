...
function Certificate({ result, event, race, catPosition }: Omit<Props, 'onClose'>) {
    ...
    return (
        <div
            className="relative border-2 border-brand-200 rounded-xl overflow-hidden"
            style={{ background: 'linear-gradient(135deg, #f0f7ff 0%, #ffffff 50%, #f0f7ff 100%)' }}
        >
            {/* Top accent */}
            <div className="h-2" style={{ background: 'linear-gradient(90deg, #7a2510, #e8430a, #fb8038)' }} />
            ...
            <div className="px-8 py-7">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <p className="font-display font-800 text-lg text-brand-700 leading-none">RunTiming</p>
                ...
                <div className="text-center mb-6">
                    <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-brand-50 border-2 border-brand-200 mb-3">
                ...
                {isFinisher && (
                    <div className="grid grid-cols-3 gap-3 mb-6">
                        <div className="text-center bg-white border border-slate-200 rounded-xl py-3">
                            ...
                            <p className="font-display font-800 text-2xl text-brand-700">{positionLabel(result.position)}</p>
                        </div>
                        {catPosition !== undefined && (
                            <div className="text-center bg-white border border-slate-200 rounded-xl py-3">
                                <p className="text-xs text-slate-400 mb-0.5">Categoria {result.category}</p>
                                <p className="font-display font-800 text-2xl text-brand-700">{positionLabel(catPosition)}</p>
                            </div>
                        )}
                        <div className="text-center bg-white border border-slate-200 rounded-xl py-3">
            ...
            {/* Bottom accent */}
            <div className="h-1" style={{ background: 'linear-gradient(90deg, #7a2510, #e8430a, #fb8038)' }} />
        </div>
    );
}
